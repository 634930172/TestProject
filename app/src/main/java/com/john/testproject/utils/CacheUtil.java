...
public class CacheUtil {
    ...
    /**
     * 清除Glide图片所有缓存（此方法为默认路径） 目录改成Glide缓存的路径
     */
    public void clearGlideImageAllCache(Context context) {
    ...
    /**
     * 获取Glide目录下缓存文件的大小（此方法为默认Glide的默认缓存路径） 项目中改成自己Glide缓存的路径
     *
     * @return CacheSize
     */
    ...
    public String getCacheSize(Context context, String CachePath) {
        try {
            return getFormatSize(getFolderSize(new File(CachePath)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
     ...
     * @return CacheSize
     */
    public String getCacheSize(Context context, File CacheFile) {
        try {
